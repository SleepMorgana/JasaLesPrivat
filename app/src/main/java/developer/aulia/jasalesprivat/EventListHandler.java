...
//	}
    ...
    public static boolean createStaticEvent(String location, Calendar startTime, Calendar endTime,
        ...
        StaticEvent staticEvent = new StaticEvent(location, startTime, endTime, isStatic,
...
//        }
        ...
        if (!check) {
            ...
            try {
                ...
                Log.e("error in check", e.toString());
            }
        ...
        while (!currStaticEList.isEmpty()) {
            ...
            if (!currStaticEList.isEmpty()) {
                ...
                if (firstCheck.getEndTime().compareTo(secondCheck.getStartTime()) >= 0) {
                    ...
                    StaticEvent newevent = new StaticEvent(firstCheck.getLocation(),
    ...
    private static boolean updateFreeTime(PriorityQueue<StaticEvent> sortedStaticEList, PriorityQueue<DynamicEvent> currDynamicEList,
        ...
        for (int i = 0; i <= days; i++) {
            ...
            freeBlock = new StaticEvent("null", startTime, endTime,
                ...
                if(temp.getStartTime().get(Calendar.YEAR) == freetime.getStartTime().get(Calendar.YEAR)
                    ...
                    //when we can actually start planning the freetime. when the static event start after the free time starts
                        ...
                        if (temp.getEndTime().compareTo(freetime.getEndTime()) > 0) {
                            ...
                            freetime2 =  new StaticEvent("null", Calendar.getInstance(), Calendar.getInstance(),
                                    true, false, false, "null", "null");
    ...
    public static Calendar getLastDeadline() {
        DynamicEventList deadlineList = EventListHandler.getDeadlineList();
        ArrayList<DynamicEvent> list = deadlineList.getList();
        Calendar lastDeadline = Calendar.getInstance();
        for (DynamicEvent event : list) {
            Calendar curr = event.getDeadline();
            if (curr.after(lastDeadline))
                lastDeadline = curr;
        }
